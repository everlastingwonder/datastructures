...
void main(void)
{
	Queue_type emergency, landing, takeoff;
	Queue_type *pe = &emergency;
	...
	for (curtime = 1; curtime <= endtime; curtime++) {
		...
		if (!Empty(pe)) {			/* emergency landing	*/
			DeleteQueue(&plane, pe);
			EmergencyLand(plane, curtime, &nland, &ncrashed);
		} else if (!Empty(pl)) {	/* bring plane to land	*/
			DeleteQueue(&plane, pl);
			Land(plane, curtime, &nland, &landwait, &ncrashed);
	...
	Conclude(nplanes, nland, ntakeoff, nrefuse, landwait, 
			 takeoffwait, idletime, endtime, ncrashed, pt, pl);
}
